...
load("~/Desktop/DESpace_extention/muSpaData_Data/Wei22_example.rda")
...
colData(spe_example) |> head()
```

The spatial tissues of each sample were annotated via Banksy [@Banksy], classifying cells into five domains. These domain annotations are stored in the `Banksy_smooth` column of colData. Additionally, the columns `sdimx` and `sdimy` contain the spatial coordinates of the cells, while the `condition` column specifies the group (i.e., stage) each cell belongs to.
...
```{r DESpace}
set.seed(123)
results <- dsp_test(spe = spe_example,
...
In other words, we are testing whether the spatial structure of gene expression (summarized by the domains) differs between 2 and 10 DPI.
...
(feature <- results$gene_results$gene_id[seq_len(1)])

# Extract the gene_name by matching the gene_id
(feature_name <- rowData(spe_example)$gene_id[
  rowData(spe_example)$gene_name %in% feature
])
```
...
plots <- lapply(sample_ids, function(sample_id) {
  ...
  # Subset spe_example for each sample
  spe_j <- spe_example[, colData(spe_example)$sample_id == sample_id]
...
set.seed(123)
cluster_results <- individual_dsp(spe_example,
                                  cluster_col = spatial_cluster,
...
`logFC.condition20DPI.cluster_id2` represents the interaction between the 20 DPI condition and cluster 2. It compares the effect of 20 DPI in cluster 2 with its effect in all other domains (i.e., all tissue regions excluding cluster 2, which serves as the baseline).
...
# Extract the gene_name by matching the gene_id
(feature_name <- rowData(spe_example)$gene_id[
  rowData(spe_example)$gene_name == feature
])
```
...
Under the null hypothesis, gene expression changes across conditions are consistent across clusters. In other words, connecting the expression levels at each time point for the same cluster should result in nearly parallel lines. 

The figure shows two lines representing the average CPM for cluster 2 and for all other clusters (excluding cluster 2). These lines deviate from parallelism: abundance is higher in cluster 2 at 2 DPI but decreases at 10 DPI, while the average abundance across other clusters remains relatively constant over time.

TODO: CPM non-linear effect; logCPM; account for domain size
...
```{r class.source = 'fold-hide'}
# calcualte cpm
>>>>>>> 90440f19c604228f7dc33362653371fd17bd2f1d
cps <- cpm(results$estimated_y)
...
# calculate average CPM for cluster 2 and all other domains, excluding cluster 2
m_summary <- mdata |>
    group_by(day, Banksy_smooth) |>
    summarise(avg_cpm = mean(cpm), .groups = "drop") |>
    mutate(cluster_type = ifelse(Banksy_smooth == "2", 
                                 "Avg 2", "Avg (excl. 2)")) |>
    group_by(day, cluster_type) |>
    summarise(avg_cpm = mean(avg_cpm), .groups = "drop")  

# plot
ggplot(mdata, aes(x = day, y = cpm)) +
    geom_jitter(aes(color = Banksy_smooth), size = 2, width = 0.2) +  
    geom_line(data = m_summary, 
              aes(y = avg_cpm, group = cluster_type, linetype = cluster_type)) +  
    scale_x_continuous(breaks = c(2, 10, 20)) +  
    labs(title = feature_name, x = "Days post injury", 
         y = "Counts per million (CPM)", color = "Banksy domain") +
    theme(legend.position = "right") +
...
Alternatively, we can visualize the gene expression at physical space. 
...
metadata <- expand.grid(sample_id = levels(spe_example$sample_id),
                        cluster = levels(spe_example$Banksy_smooth)
                        ) |>
...
One significant gene:

```{r}
(gn <- results[["gene_results"]][["gene_id"]][9])
w <- results[["estimated_y"]][["genes"]][["genes"]] == gn

# Extract the gene_name by matching the gene_id
(gn_name <- rowData(spe_example)$gene_id[
  rowData(spe_example)$gene_name == gn
])
```
...
To explore predicted counts based on estimated coefficients, we calculate and visualize the CPM values for `r gn_name`. 

```{r}
fitted_values <- results[["glmFit"]][["fitted.values"]]
m <- melt(fitted_values[w,]) |>
...
The lines represent the average CPM for domain 0 and domain 4. 
The expression levels of `r gn_name` in cluster 0 first increase and then decrease, while in cluster 4, the trend is the opposite, with a decrease followed by an increase.
...
# calculate average CPM for cluster 2 and all other domains, excluding cluster 2
m_summary <- mdata |>
    filter(Banksy_smooth %in% c(0, 4)) |>
    group_by(day, Banksy_smooth) |>
    summarise(avg_cpm = mean(cpm), .groups = "drop") |>
    mutate(cluster_type = ifelse(Banksy_smooth == "0", 
                                 "Avg 0", "Avg 4")) |>
    group_by(day, cluster_type) |>
    summarise(avg_cpm = mean(avg_cpm), .groups = "drop")  
...
ggplot(mdata, aes(x = day, y = cpm)) +
    ...
    geom_line(data = m_summary, 
              aes(y = avg_cpm, group = cluster_type, linetype = cluster_type)) +  
    scale_x_continuous(breaks = c(2, 10, 20)) +  
    labs(title = gn_name, x = "Days post injury", 
         y = "Counts per million (CPM)", color = "Banksy domain") +
    theme(legend.position = "right") +
    guides(color = guide_legend(byrow = TRUE), 
           linetype = guide_legend(title = NULL))
...
Visualize the expression of the top gene, `r gn_name`, across samples.
...
plots <- lapply(sample_ids, function(sample_id) {
    ...
    spe_j <- spe_example[, colData(spe_example)$sample_id == sample_id]
    # Create FeaturePlot for the sample
    plot <- FeaturePlot(spe_j, feature = gn, 
                        cluster_col = spatial_cluster,
                        coordinates = coordinates, 
                        platform = "Stereo-seq",
                        point_size = 0.01,
...
The trend aligns with the model's prediction: gene abundance in cluster 0 increases at later stages (10 DPI) compared to the earlier stage (2 DPI) and other clusters.
