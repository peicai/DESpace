...
To explore predicted counts based on estimated coefficients, we calculate and visualize the fitted values for `r gn_name`.
...
The expression levels of `r gn_name` in cluster 0 increase over time, while other clusters show relatively stable expression patterns across different days post-injury.

```{r}
p1 <- ggplot(m, aes(x=day, y=fitted, group=cluster, colour = cluster)) +
...
The trend aligns with the model's prediction: gene abundance in cluster 0 increases at later stages (10 and 20 DPI) compared to the earlier stage (2 DPI) and other clusters.
